...
class ReadersDetail(models.Model):
    ...
    # T00316 Created is reader boolean field to check reader is a member of library or not
    is_reader = fields.Boolean(default=False, readonly=True)

    # T00316 Created reader ids field to get borrowed book id
    reader_ids = fields.One2many("borrow.books", "reader_id")

    # T00316 Created total borowed book field to get count of borrowed books
    total_borrowed_book = fields.Integer(compute="_count_book", store=True)

    # T00316 Created count field to get the count of total books in library
    count = fields.Integer(compute="_count_books")
    get_borrowed_books = fields.Char(compute="get_books")

    # T00316 Created priority widget to give rating to the readers
    ...
    # T00316 Created state fields to check the status of readers
    ...
    def get_books(self):
        """This method will show borowed book detail if there is one book borrowed then
        it will show form view and if more than one book borrowed then it
    ...
    def _onchange_borrowed_books(self):
        """Created the onchange decorator to check wheather a reader is member of
        library or not #T00316"""
        for rec in self:
    ...
    def _count_book(self):
        """Created count book compute method to get count of
        total borrowed books #T00316"""
        for book in self:
            book.total_borrowed_book = len(book.reader_ids)

    def _count_books(self):
        """Created count books compute method to get
        count of total library books #T00316"""
        count = 0
